...
public class WallDetector : MonoBehaviour
{
    ...
    [SerializeField] private float doubleRayOffset = 0.1f; // Offset for double ray system
    [SerializeField] private LayerMask wallLayers = -1;
    // REMOVED: groundLayers
    
    [Header("Ray Configuration")]
    [SerializeField] private float rayHeight = 1f; // Height offset for wall detection rays
    
    [Header("Wall Running Requirements")]
    // REMOVED: minHeightAboveGround
    [SerializeField] private float minWallAngle = 80f; // How steep wall needs to be
    [SerializeField] private float minWallNormalAngleChange = 15f; // To detect "new" walls
    ...
    void ScanForWalls()
    {
        // CHANGED: Add rayHeight to origin
        ...
        // REMOVED: HEIGHT REQUIREMENT CHECK
        // Since wallrunning only initiates when jumping/falling, we don't need ground check
        ...
        if (primaryWallType != WallType.None)
        {
            ...
            // CHANGED: Simplified canWallRun logic - removed height check
            canWallRun = wallAngle >= minWallAngle && 
                        (hasLeftWall || hasRightWall); // Only side walls for wallrunning
        }
        ...
        CurrentWallData = new WallData
        {
            ...
            hasFrontWall = hasFrontWall,
            hasBackWall = hasBackWall,
            // REMOVED: isHighEnough (always true now)
            isNewWall = isNewWall,
    ...
    void OnDrawGizmos()
    {
        ...
        // REMOVED: Ground check hit point visualization
        ...
        {
            Gizmos.color = new Color(0f, 1f, 0f, 0.1f); // Transparent green
            Gizmos.DrawWireSphere(transform.position, 1f); // Use transform.position, not origin
        }
        ...
        {
            Vector3 labelPos = transform.position + Vector3.up * 2f;
            string reason = data.wallAngle < minWallAngle ? "Wall Too Shallow" : "No Side Wall";
            UnityEditor.Handles.Label(labelPos, $"Can't Wall Run\nReason: {reason}");
        }
        #endif
    }
